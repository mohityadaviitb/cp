...
void prllarr(ll arr[], ll n){fu(i, 0, n) cout << arr[i] << " ";cout << "\n";}
...
ll stringtoll(string s){return stoi(s);}
string lltostring(ll n){return to_string(n);}
string decToBinary( ll n){string s=""; ll i = 0;while (n > 0) {s =to_string(n % 2)+s;n = n / 2;i++;}return s;}




ll countDistinctElements(const std::vector<ll>& vec) {
    // Use a set to store distinct elements
    std::set<ll> distinctElements(vec.begin(), vec.end());
    // The number of distinct elements is the size of the set
    return distinctElements.size();
}

ll findMostFrequentElement(const std::vector<ll>& vec) {
    ...
    ll maxFrequency = 0;
    ll mostFrequentElement = vec[0];
    
    for (const auto& pair : frequencyMap) {
        if (pair.second > maxFrequency) {
            maxFrequency = pair.second;
            mostFrequentElement = pair.second;
        }
    }
...
// while(ans>1){
    ...
    ll re=1;
        ...
        ll x= countDistinctElements(a);
        ll y= max(re,findMostFrequentElement(a));
